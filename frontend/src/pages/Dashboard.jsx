...
export default function Dashboard() {
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-text-primary">Overview</h2>
          {user && (
            <div className="flex items-center gap-2 mt-1 text-text-secondary text-xs md:text-sm">
              <span className="material-symbols-outlined text-sm text-primary">location_on</span>
              <span>Showing data for <strong className="text-text-primary">{user.city}</strong></span>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div onClick={() => navigate('/admin/alerts')} className="cursor-pointer sm:col-span-2 lg:col-span-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <Card className="lg:col-span-2 flex flex-col gap-6 overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex flex-col">
              <h3 className="text-base md:text-lg font-bold text-text-primary">Pollution Trends vs Complaint Volume</h3>
              <p className="text-xs md:text-sm text-text-secondary">30-day comparative analysis</p>
            </div>
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                <span className="size-2 rounded-full bg-primary"></span>
                <span className="text-[10px] md:text-xs font-medium uppercase text-text-secondary">Pollution</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="size-2 rounded-full bg-text-muted"></span>
                <span className="text-[10px] md:text-xs font-medium uppercase text-text-secondary">Complaints</span>
              </div>
            </div>
          </div>
        ...
        <Card className="flex flex-col gap-6 h-full">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-text-primary">Nearby Alerts</h3>
          </div>
          <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar max-h-[400px]">
            {alerts.length === 0 ? (
              <p className="text-sm text-text-secondary text-center py-4">No active alerts</p>
            ) : (
              alerts.map(alert => (
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-3">
            <Button variant="outline" className="text-xs md:text-sm flex-1" onClick={() => navigate('/admin/ai')}>
              <span className="material-symbols-outlined text-sm">robot_2</span>
              AI Assistant
            </Button>
            <Button variant="primary" className="w-full text-xs md:text-sm flex-1">Generate Strategy Report</Button>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="p-4 md:p-6 border-b border-border dark:border-border flex items-center justify-between">
          <h3 className="text-base md:text-lg font-bold text-text-primary">Recent Compliance Incidents</h3>
          <button 
